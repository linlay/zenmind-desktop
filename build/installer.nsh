...
!macro customCheckAppRunning
  ...
  ${if} $R0 == 0
    DetailPrint "Requesting ZenMind to exit before installing..."
    ${if} ${FileExists} "$INSTDIR\${APP_EXECUTABLE_FILENAME}"
      nsExec::ExecToLog `"$INSTDIR\${APP_EXECUTABLE_FILENAME}" --zenmind-shutdown-for-update`
      Pop $R2
      Sleep 500
    ${endif}
    ...
    waitZenMindExit:
      ...
      ${if} $R1 < 12
        Sleep 500
        Goto waitZenMindExit
      ${endif}

      DetailPrint "Force closing ZenMind before installing..."
  ...
  !insertmacro stopZenMindManagedProcesses
!macroend
