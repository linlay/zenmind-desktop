...
打包时设置 `DEMO=1` 或 `DEMO=true` 后，内置 `demo-node-html` 模板会在启动时复制到 `data/webs/webapps/demo-node-html/`；目标已存在时按安装包内模板强制刷新。未设置 `DEMO` 时安装包不包含 demo，启动时也不会创建 demo 网站应用。运行时只承认 `data/webs/`、`config/webs/`、`state/webs/` 和 `logs/webs/`。网站自身的浏览器数据不保存在 manifest 中。cookie、localStorage、IndexedDB、webview session 数据和缓存由 Electron/Chromium 管理，位于：
