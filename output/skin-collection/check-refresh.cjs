...
app.whenReady().then(async () => {
  ...
  for (const entry of report.filter(e => ['tahiti.skin.zip','gold-saints.skin.zip'].includes(e.file))) {
  ...
  for (let i = 0; i < report.length; i++) {
    if (!['tahiti.skin.zip','gold-saints.skin.zip'].includes(report[i].file)) continue;
