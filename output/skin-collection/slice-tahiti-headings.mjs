...
for(const [m,mode] of ['light','dark'].entries()){
 const im=await decode(fs.readFileSync(`${root}/tahiti-artwork/headings-${mode}.png`));
 pc.fillStyle=mode==='light'?'#E9FAFC':'#102C33';pc.fillRect(m*560,0,560,560);
 const rows=(mode==='light'?[0,340,650,955,1280]:[0,330,630,920,1280]).map(y=>Math.round(y*im.height/1280));
 for(const [r,group] of ['pinned','chats','projects','websites'].entries())for(const [col,locale] of ['zh-CN','en-US'].entries()){
  const c=createCanvas(im.width/2,rows[r+1]-rows[r]),ctx=c.getContext('2d');
  ctx.drawImage(im,col*c.width,rows[r],c.width,c.height,0,0,c.width,c.height);
  ...
  fs.writeFileSync(`${root}/tahiti-artwork/${mode}-${group}-${locale}.png`,out.toBuffer('image/png'));
...
fs.writeFileSync(`${root}/tahiti-headings-preview.png`,preview.toBuffer('image/png'));
