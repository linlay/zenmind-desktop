...
- macOS 在侧栏空白处双击，确认进入系统原生全屏，侧栏与当前页面保持显示；再次双击同一区域退出全屏并恢复窗口。顶部拖动带和浏览器工具栏空白仍双击最大化 / 还原。
...
- macOS 已通过 traffic lights 或快捷键进入系统全屏时，侧栏空白仍可双击退出；全屏中的拖动不得移动窗口，拖动后点击不得误退出全屏。点击 WebClient 内容后立即回到侧栏双击和拖动也正常。
- 系统全屏中其他拖动区、WorkPanel 全屏及其切换期间、搜索或 guest 模态遮罩显示时，双击不得切换主窗口状态。原生全屏动画期间重复双击不得提交第二次转换；先系统全屏再进入 / 退出 WorkPanel 全屏，应保留原有系统全屏。
...
- macOS 与 Windows 分别在 240px 最窄侧栏和 360px 侧栏检查“对话”：展开箭头始终在文字左侧，展开时默认助手以固定紧凑宽度显示；从栏外移入标题、箭头、助手及排序/新建按钮时，各点击区域不移动。分别点击箭头和标题只切换展开状态，点击助手只打开选择菜单；用键盘展开、收起和打开/关闭助手菜单，并检查中英文、长助手名及浅深色下无重叠。
