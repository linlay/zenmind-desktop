...
- macOS 与 Windows 分别在浅深色、展开和收起侧栏检查滚动条：初始和静止时隐藏，滚轮、触控板、键盘导航引发滚动或拖动滚动条时显示，停止滚动约 0.8 秒后隐藏；仅悬停侧栏不显示，连续滚动不会提前隐藏，显隐时列表宽度和文字位置不跳动。
...
- 桌面便签 v0.3：首次打开创建一张独立纸张窗口；从“＋”新增另一张，两张同时输入并自动保存，关闭一张不关闭另一张或停止后端。确认标题、正文、待办和中文输入正常，颜色菜单、置顶、所有便签入口、备份与归档恢复可用；收起不删除数据，永久删除前确认。
- macOS 检查圆形关闭控件、拖动条、原生窗口缩放与 floating 置顶；Windows 检查右侧最小化/关闭、拖动及边缘缩放、topmost 置顶，100%/150%/200% 缩放下无内容截断。关闭重开和 Desktop 正常重启后恢复各张的位置、大小、颜色、置顶与正文；拔掉外接屏幕后仍能找到窗口。
- 延迟或拒绝单张便签保存，关闭时保留编辑或明确放弃，恢复连接后重试；两张不同便签互不冲突，同一张的过期写入显示冲突而不覆盖。停止或卸载插件回收全部便签窗口。自动化入口为 `electron qa/sticky-notes-native-smoke.cjs <便签ZIP绝对路径> <截图输出目录>`；Windows 实机行为仍需按上项检查。

- macOS / Windows 分别通过插件中心导入便签 ZIP，确认自动初始化成功且首次导入保持停止；启动后打开便签并保存内容，停止、重新启动和重新导入后内容保留。对已出现初始化失败的安装直接重试初始化，确认可恢复且不要求删除数据。重复市场安装、更新与卸载，确认没有 Services/Webs 集成端口缺失错误；初始化失败不新增市场已安装记录，停止失败不删除插件程序和资源。
- macOS / Windows 分别打开独立 WebApp 窗口，在正文右键启用/关闭“保持置顶”，确认只影响当前窗口；开启“小窗模式”后可缩放到便签尺寸，再关闭该选项恢复原有边界与最小尺寸。全屏时小窗选项禁用；窗口关闭重开后回到默认窗口偏好。
- 延迟便签的保存请求后立刻关闭独立窗口，确认宿主显示未保存提示，“继续编辑”保留 guest 和输入，“仍然离开”关闭窗口；保存完成后正常关闭不再提示。显式停止/卸载和 Desktop 退出仍可强制回收，不被页面 `beforeunload` 卡住。
- 在独立 WebApp 的文本编辑区右键，确认撤销、剪切、复制、粘贴和全选仍作用于真实 guest；网站应用页面无新增窗口 IPC 或 popup 能力。用桌面便签新建正文/待办、切换颜色、归档、导出备份、关闭重开和导入备份，确认内容恢复，未保存错误明确提示，同 ID 的不同内容保留为导入副本。
