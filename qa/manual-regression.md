...
- 从 Sidebar 依次打开不同 Agent 的多个历史 Chat，再通过 Cmd+K、新建按钮触发 New Chat 和继续对话；确认 Desktop route、Main Chat guest URL、WebClient Router、页面内容与 Registry owner 一致切换，`webContentsId` 保持不变，变化的 `chatId/newChat` 会经 route bridge 到达 WebClient，而不是只改变地址栏或外层选中态。地址栏已经到达目标但 Router 尚未切换时，bridge 仍必须让 Router 完成 replace；日志中同一 revision 必须依次出现 `chat-route-bridge-queued → main-chat-router-ack-received → main-chat-router-ack-accepted`，正常路径不得执行 Main Chat `history.pushState` 或 reload。150ms 未收到 ACK 时只能记录 `main-chat-router-ack-retry` 并重发同一 revision，不得 reload；人为持续阻断 ACK 时，即使物理 URL 已匹配也必须在首次投递累计 1000ms 后记录 `main-chat-router-ack-timeout` 并且只执行一次 `loadURL`；错误 revision、Router location 或 guest 的 ACK 只能记录 rejected，不能取消兜底。在历史 Chat A/B 分别输入未发送草稿，多次切换后必须分别恢复；在 New Chat 输入草稿后切到历史 Chat，再从相同或不同 Agent 返回 New Chat，必须恢复同一份共享草稿，发送后才清空。Inspector 中 Primary 物理 WS 全程保持同一条；旧 Chat 有 active Run 时只出现一次 detach，无 active Run 时不产生 detach，切换后的 `/api/chat` 仍在原 LogicalSession/Primary WS 上完成。Realtime 诊断中每次稳定 Chat 切换只允许一次目标 `chatId` 加载，不得在 surface active 恢复时紧随发出旧 `chatId` 的 `forceReload`。
...
- 展开桌宠“对话概览”，确认仅显示七天内的 unread 与 awaiting 会话，视窗完整容纳三条并可用滚轮继续浏览；chat name 与正文均为 13px，item 间有清晰的 1px 分割线，unread 为蓝点、awaiting 为橙色时钟且不显示回复入口。关闭按钮默认不占位且仅在 item hover/focus 时叠加出现；普通 unread 可展开单行回复，打开对话、回复成功或关闭后 unread 在桌宠中转为 read，单纯 hover、滚动和展开列表不得标记已读。
