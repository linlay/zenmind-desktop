...
- 测试市场发布 `type: connector` 的企业微信条目后，Desktop 的连接器页显示相同名称、描述和详情；已有 `flowcenter` 等旧 MCP 安装记录不混入云端连接器目录。搜索企业微信可找到条目，空目录不以旧安装记录填充；无当前平台制品时卡片仍可浏览并提示平台不可用。详情入口打开对应商品，连接器中心入口用于导入、配置和授权，不触发旧 MCP/Skill 安装器。

- macOS 与 Windows 分别检查技能、连接器、网站应用三个页签，以及中英文、窄窗口布局；技能和技能包卡片尺寸一致，技能包保持堆叠外观但顶部没有额外边框。头像优先使用市场图标，缺失时使用稳定的彩色回退图标。
- 精选只出现市场明确标记的条目；无精选时隐藏精选区域，不按下载量补选。点击“换一换”轮换条目；技能/技能包和分类切换、搜索结果均正确。
...
- macOS / Windows 登录后模拟换票 401/403 或跳转登录页：左下角立即显示未登录、旧凭据被撤销，市场公共条目仍可浏览，用户态操作要求重新登录。模拟 token 到期且换票 503/断网：显示未登录，网络恢复后可后台恢复；尚未到期的 token 提前刷新遇到 503 不应误退出。
...
- 分别从对话右键菜单和顶部分享图标打开弹窗，确认两者是同一套 560px 平面化弹窗，遮罩、边框、圆角、背景、阴影和 Cmd+K / Cmd+H 一致；默认仅显示有效期与创建操作，“分享说明”收起且可通过鼠标和键盘展开，弹窗不显示底部 Done 操作；窄窗口下正文独立滚动且标题栏保持可见。
...
- 图片缩放控件在预览/编辑态一致：缩小、百分比与单一下拉箭头、放大；原始比例从下拉菜单的 100% 选择，不再单独显示 1:1 按钮。输入 150 后 Enter 或失焦才应用，清空/非法输入恢复当前值，Esc 取消，越界输入限制到 10%–800%；预设含适合窗口与 100%，边界缩放按钮禁用。连续使用 Cmd/Ctrl +/- 必须逐档变化，输入框内 Cmd/Ctrl+A/Z 保持原生编辑行为；触控板缩放不改变整个应用比例。macOS/Windows、浅深色、窄面板与全屏下无双箭头、数字微调器或控件重叠。
...
- macOS 与 Windows 分别模拟 query/attach 返回 `409 active_stream_exists`：聊天错误正文直接说明上一条运行流尚未释放，不必展开 Technical details；详情保留原始错误码与 409。模拟未知错误码时正文直接显示服务端原因，发送失败保留输入草稿，不自动重发 query。
...
- macOS 与 Windows 分别在浅色、深色下打开 Website 和 WorkPanel 网页：未声明背景的网页区域应保持白底，黑字清晰，工具栏跟随主题；显式深色背景的网页保持自己的背景。可用天猫国际商品详情页对照，同时确认淘宝首页仍为白底、本地透明 WebApp 仍能透出壁纸，切换主题不重建 guest。
...
- macOS / Windows 在浅色与深色下切换默认、森林、海洋等皮肤及自定义图片：有图片时 Kanban 页面、顶部栏与列通过一层随明暗主题变化的 88% 底色遮罩隐约透出同一 Desktop 壁纸（自定义照片也使用同一遮罩），任务卡片及拖拽卡片保留阅读底色，详情与新建弹窗使用统一浮层表面色，详情中的 Chat 保持实色。滚动看板不移动壁纸；切回默认无图片皮肤后恢复实色，不残留上一皮肤颜色。
...
- 点击底部 Update 下载时若安装包 404 或校验失败，弹出错误对话框；关闭后 Update 按钮仍保留且可重试。尚未发现更新时检查接口失败不显示外部更新入口；后台检查错误不主动弹窗。
- 更新按钮默认 20 × 20px，下载图标和文字均为 12px；展开侧栏 hover 时按钮宽度扩展为 64px、高度保持 20px，完整显示 Update／更新。

- 新用户默认不自动下载；有效新版本清单返回后仅显示更新入口，点击底部齿轮位置的更新按钮才下载。安装包 404 时设置页显示下载失败，菜单保留更新卡片和下载重试入口。检查带 v 前缀的本机版本（如 v0.4.5）能正常比较，远端版本仍严格校验。About 更新卡片应保留内边距，仅自动下载选项上方有分隔线。

- 已有更新时发生错误，底部菜单仍保留更新卡片，设置旁及 About 根据失败阶段提供下载或升级重试；清理或原生安装失败提示重启恢复服务，缓存包保留。尚无更新元数据时不新增外部更新入口。有可用更新及下载/校验/待安装期间，下载图标直接替换底部齿轮，不额外显示角标或挤占账号文字；中英文、登录/未登录、侧栏展开/收起均验证。
...
- 市场只展示正式 connector 商品及实际本地自定义连接器；旧 MCP 记录不能冒充商品。已安装状态从 Platform 回读，离线不伪造已安装。
- 未授权企业微信点击“连接并启用”依次准备私有 CLI、打开扫码、回读授权、启用并挂载默认 Agent；中途取消或离开市场，扫码进程结束，迟到请求不能恢复绑定。
- 多步骤授权 URL 改变后更新同一授权窗口；关闭窗口、网络失败、到期和重试均不留下后台登录进程。
- 停用后保留登录，重新启用不要求重复扫码；解绑后 bound/enabled 均为 false，停止业务进程并清理当前用户凭据，共享安装程序保留。
- 自定义远程 MCP 支持无认证、MCP OAuth、API 密钥；ZIP 导入和高级清单走同一个 Platform 验包入口。仅保存不自动授权，保存并连接延续统一流程。
- Token 新值先执行 CLI status 或全部 MCP 初始化/工具发现，验证失败仍可使用旧凭据；无独立检查的 CLI 明确显示“已配置，待验证”。
- Desktop 切换账号后看不到上个账号的连接器授权；检查授权、启停、解绑和后台刷新均使用当前主体。测试账号不得覆盖用户现有机器人的登录材料。
...
## Windows 技能与 Node 运行时事务回归

- Platform 开启热重载，市场普通技能首次安装、覆盖更新、卸载均成功；正式目录由 Platform 发布，Desktop 不出现本地 preserveBackup 重命名错误。
- 本地 ZIP/目录/命令导入使用相同入口；Platform 不可用或不支持条件事务接口时旧技能完整保留，无直接文件写入降级。
- 注入市场安装记录写入失败，确认旧技能通过 Platform 恢复；补偿失败或请求结果不确定时提示恢复包路径，保留恢复包。
- 快照取得后通过另一管理请求编辑同一技能，原替换/删除请求应收到 revision 冲突；在市场记录失败、补偿请求发出前插入另一次编辑或重新创建，补偿不得覆盖该新内容。确认恢复 ZIP 包含空目录、执行权限与隐藏配置，Desktop 不读取正在使用的正式技能目录。
- 技能包与 MCP/CLI 连接器在 watcher 开启时安装、覆盖、删除；使用中拒绝规则和授权数据保持，操作后手动编辑仍触发热重载。
- 首次启动先完成 env 和 Node/npm 准备，再并发启动 identity-center、agent-platform 和 agent-webclient；Node/npm 准备失败时三个服务均不进入启动阶段，修复资源后可重试。普通用户权限、含中文和空格的路径下，手动启动 Platform 与一键启动复用同一 Node 准备结果；npm 短时 EPERM 可恢复，持续占用有界失败，旧运行时保留。
- Node 运行时发布失败可恢复旧版；清理失败不遮蔽原始错误，已成功发布时不误报服务准备失败；重新启动可修复不完整缓存。
- 记录 Windows 实机执行结果；macOS 测试及交叉编译不能代替实机验收。
...
- 连接器按实例共用配置完成和授权状态；切换市场登录身份只刷新可见目录与收藏，不更换连接器凭据。
- 取消/成功/失败提示使用 message；连接器错误按条目隔离，重试执行原操作；恢复就绪后清除对应准备/授权错误。
- 外部浏览器授权等待三分钟后释放界面，取消精确会话，迟到响应不能挂载。
- 自定义 MCP JSON 保存走 Platform 导入，拒绝重复键/明文秘密，展示真实安装结果。
- 市场安装技能后从技能中心删除，返回市场显示未安装；内置 builtin-dbx/httpx 不进入我安装的列表和数量。
- 指定个人/多人资源使用当前 SSO 身份读取目录和技能详情；切换账号清除旧数据及在途响应。

### WorkPanel 网页浏览器工具

- macOS / Windows × 明 / 暗主题：普通网页的 Edit 改为批注图标，支持高亮切换、悬停说明；加载中与非 HTML 页面禁用。退出批注后草稿保留。
- 更多菜单提供查找、打印、缩放、默认浏览器打开和复制链接；缩放连续点击保持菜单打开，边界为 25% / 300%，重置为 100%；刷新与切换标签保持站点缩放，Desktop 工具栏尺寸不变，批注框跟随元素。
- 主面板及独立窗口分别验证 Cmd/Ctrl+F、P、+、-、0，焦点位于网页内部和地址栏时均生效；切换 Chat / tab 后不得作用于隐藏或旧 guest。查找验证中文、无匹配、前后匹配、Enter / Shift+Enter、Esc 关闭、导航后清理。
- 打印打开系统对话框，取消可继续操作；打印内容不包含批注框与选择层。独立窗口菜单同样支持查找、打印、缩放和链接操作，关闭窗口还原后继续可用。

### 当前 Platform 连接器契约

- 状态读取消费 configured/canCheck；连接器详情不提供独立启停，不向 connection 发送 PUT。Agent 挂载仍使用独立的 enabled 参数。
- 首次 Token 待验证不能进入可用对话；旧凭据仍可用、新凭据待验证时同时展示两者。显式检查完成后刷新状态，取消/超时后不得继续挂载。
