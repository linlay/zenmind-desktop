...
- [ ] Main Chat 仍是唯一上游 live surface；WorkPanel Overview/Debug 首次打开先做一次 HTTP replay，再以 `lastSeq` 订阅 Main Chat 当前 visible Run 的 Desktop 本地镜像。macOS/Windows 从 Chat A 切到运行中的 Chat B 时，HTTP replay 内容不会约 1 秒后变成 `target_unavailable`，Realtime Inspector 保持同一个逻辑 Session 和单一上游 observer；若桌宠等内部 consumer 已观察 Chat B，先看到旧 observer 的 `/api/detach` 与 `reason=detached`，再看到 Main Chat `/api/attach`，且 Run 保持 active、不出现 `completed Run cannot become visible again`，Retry 也不被本地假终态卡死。主聊天后续 Planning、Plan Task、File Change 与内容事件无需切换标签即同步。抓包确认 Overview/Debug 不新增 Platform query/attach/detach 或轮询，隐藏时只释放本地 consumer，再显示时 replay/虚拟订阅恢复且不丢不重。构造超过 2000 条或 4 MiB 的本地 replay 后用旧游标打开 Overview，首次 `seq_expired` 自动刷新 `/api/chat` 并重新订阅；持续过期时停止自动刷新、显示本地化错误和手动重试，Realtime Inspector 中错误包含 `requestedLastSeq`、`firstAvailableSeq`、`latestSeq`、事件数与字节数，且全程不出现第二条上游 observer。
...
- [ ] 在同一个 WS v2/Frame Port v2 逻辑 Session 中完成第一轮新 Chat 后，快速连续发送第二条消息并人为延迟 canonical owner 登记：guest 已是 `?chatId=...` 时第二条 query 最多等待 1500ms，owner 登记到达后只向 Platform 转发一次且不返回 400；等待期间 Realtime Inspector 中不出现新 token/Broker 访问、旧 Run detach 或 Platform query。再让等待超时、销毁 guest、替换 generation、切换 Agent/Chat，均只返回本地 `protocol_error` 且 Platform 无后台 Chat。
- [ ] 在 canonical owner 已登记后注入一次同 generation 的迟到 ownerless canonical Surface 快照，登记结果必须为 `{ok:false}` 且 owner 保持不变；只有 Desktop 与 guest 都切到同一个精确 `agentKey + newChat nonce` 时 ownerless 重置才返回 `{ok:true}`。随后在 guest URL 仍短暂保留旧 `newChat` 时登记匹配 canonical owner，续聊仍正常。诊断只包含 route 类型、generation 与 owner 是否存在，不包含原始 chatId、nonce 或正文。
...
- [ ] macOS 与 Windows 切换到其他应用使主窗口失焦后，在顶部拖拽条、侧栏空白区或内置浏览器顶部空白区直接按住拖动；第一次按下即可同时聚焦并移动窗口。macOS Agent WebClient 顶部完整显示 8px 宿主拖拽条，浅色/深色主题下与 WebClient 背景无缝衔接，只保留居中的轻量拖拽提示且不遮挡内容；为右侧按钮保留的非拖拽区必须保持同色，不得透出透明窗口底色；点击 guest WebView 内容后，下一次从顶部拖拽条按下也必须立即移动窗口，不得只完成焦点切换；区域内控件仍可正常点击。
