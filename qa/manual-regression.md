...
- 在“设置 → 外观”选择默认/雾林，退出并重启后保持选择；切换系统明暗和语言、修改一般设置后皮肤仍保留，旧 profile 自动使用默认皮肤。读取失败有重试入口，保存失败恢复上次选择。
- 导入 PNG/JPEG（覆盖中文、空格文件名、宽图、竖图和透明图片），重启并删除原图后背景仍可用。更换皮肤保留自定义图片，“恢复皮肤背景”只移除图片覆盖；取消选择、损坏图片、超限图片和只读目录均不替换旧背景。删除已保存副本后显示可恢复提示并回退皮肤背景。
...
- Windows 分别在顶部系统栏空白、侧栏空白和浏览器工具栏空白双击，确认最大化/还原，右上角状态图标同步；覆盖不同缩放比例和混合 DPI 显示器。
...
- 默认皮肤下，浅色标签栏为 `#FFFFFF`、选中 tab 为 `#EEEEEE`，深色分别为 `#181818` / `#303030`；tab 四角均为 `10px` 圆角，上下各留 `6px`，相邻 tab 与 `+` 间隔 `4px`。在 macOS、Windows、窄面板与全屏下检查底部圆角完整、标签栏不遮挡内容、右上角面板按钮垂直居中；多 tab 横向滚动、hover、键盘焦点和关闭按钮保持可用，切换皮肤仍消费该皮肤的标签配色。
