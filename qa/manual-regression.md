...
- macOS 与 Windows 分别在 Chats、Projects 和 Pinned 检查长聊天标题：默认省略，hover 行或键盘聚焦后缓慢往返滚动，到两端短暂停留，移开且失焦后恢复；短标题不滚动。调整侧栏宽度、修改标题、展开窄栏 Popover 后重新判断溢出，状态图标与菜单不被覆盖；开启系统减少动态效果后保持静态省略和原有悬浮详情。
...
- 重启确认混合顺序恢复；缺少独立排序文件时使用默认导航布局。新置顶默认在最前，取消置顶恢复 Sites 原顺序，删除应用不显示幽灵行。Chats/Pinned Chat/Projects 的服务端顺序不受影响。
