...
const data = ready.map(item => ({ key: item.key, manifest: item.manifest, style: COLLECTION_STYLES[item.key].label }));
...
$('name').textContent=s.manifest.name;$('style').textContent=s.style+' · '+mode+' · 34 个语义槽';$('download').href=s.key+'.skin.zip';
...
$('dot').style.clipPath=v.visuals.styles.unreadShape==='paw'?'polygon(12% 52%, 2% 40%, 0% 22%, 8% 12%, 20% 15%, 26% 35%, 27% 8%, 36% 0%, 45% 4%, 49% 30%, 53% 4%, 63% 0%, 73% 8%, 74% 35%, 80% 15%, 92% 12%, 100% 22%, 98% 40%, 88% 52%, 75% 45%, 65% 50%, 89% 73%, 91% 89%, 80% 100%, 50% 89%, 20% 100%, 9% 89%, 11% 73%, 35% 50%, 25% 45%)':v.visuals.styles.unreadShape==='heart'?'polygon(50% 95%,5% 48%,0% 25%,12% 8%,30% 5%,50% 23%,70% 5%,88% 8%,100% 25%,95% 48%)':'none';
