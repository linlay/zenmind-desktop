...
is_app_running() {
  osascript -e 'tell application "System Events" to return (name of processes) contains "ZenMind Desktop"'
}
...
button returned of (display dialog "Do you also want to delete ZenMind Desktop app data?
...
if [ "$(is_app_running)" = "true" ]; then
  show_dialog "ZenMind Desktop is still running. Quit the app and run this uninstall script again."
  printf '%s\n' "ZenMind Desktop is still running. Quit it and rerun this script."
  exit 1
fi
...
printf '%s\n' "ZenMind Desktop uninstall finished."
