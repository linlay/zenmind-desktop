...
export async function RealtimeBroker_handleDesktopBridgeRequest_3(self: RealtimeBrokerMethodContext, id: string, type: string, frame: AgentPlatformRealtimeFrame) {
    ...
    try {
        ...
        if (isAwcpSnapshot && Object.keys(frame.payload).some((key) => key !== "surfaceId")) {
            throw brokerError("protocol_error", "AWCP snapshot payload only accepts an optional surfaceId");
        }
        ...
        if (isAwcpSnapshot || isAwcpInvoke) {
            ...
            result = isAwcpSnapshot
                ? await provider.awcpSnapshot(id, scope, controller.signal, surfaceId as string | undefined)
