...
import type { ConversationShareFacade } from "../conversation-share";
import {
...
export async function revealAssistantChatInFileManager(
  ...
  try {
    if (chatDirectoryExists || fs.existsSync(targetPath)) {
      options.shell.showItemInFolder(targetPath);
      return { ok: true, message: t("revealPath.shownInFileManager") };
    }
    const parentDirectory = path.dirname(targetPath);
    if (fs.existsSync(parentDirectory)) {
      const error = await options.shell.openPath(parentDirectory);
      return error
        ? { ok: false, message: t("sidebar.chat.infoRevealUnavailable") }
        : { ok: true, message: t("revealPath.openedParentDirectory") };
    }
    return { ok: false, message: t("sidebar.chat.infoRevealUnavailable") };
  } catch {
    return { ok: false, message: t("sidebar.chat.infoRevealUnavailable") };
...
export function registerAssistantIpcHandlers(ipcMain: any, options: AssistantIpcHandlerOptions) {
  ...
  ipcMain.handle("assistant.exportChatHtml", async (_event: any, chatId: string) =>
    conversationShare.exportChatHtml(chatId, platform)
  );
  ...
  ipcMain.handle("assistant.revokeChatShare", async (_event: any, shareId: string) =>
    conversationShare.revoke(shareId)
  );
