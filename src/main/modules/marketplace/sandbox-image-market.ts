...
function createContainerHubClient(options: MarketplaceOptions, config: ContainerHubConfig) {
  if (!options.createContainerHubClient) {
    throw new Error(t("market.sandbox.requiresContainerHub"));
  }
  return options.createContainerHubClient(config);
}
...
export async function listSandboxImageMarketItems(
  ...
  try {
    const client = createContainerHubClient(options, config);
    const environments = await client.listEnvironments();
    const environmentItems = environments
      .filter((environment: ContainerHubEnvironment) => environment.available)
