...
export interface ServicesIpcHandlerOptions {
  ...
  getPluginGlobalShortcutStatuses: (serviceId: string) => any[];
  invokePluginDesktopAction: (options: any) => Promise<any>;
...
export function registerServicesIpcHandlers(ipcMain: any, options: ServicesIpcHandlerOptions) {
  ...
  ipcMain.handle("services.importEnvZip", async () => {
    ...
    try {
      ...
      // Stop, resource deployment and configuration publication must share the
      // lifecycle queue with start/restart, not only the subsequent startup.
      const imported = await runServiceMutation(async () => {
