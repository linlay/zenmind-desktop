...
import type { ServiceCapabilityPorts } from "../integration-ports";
...
function buildTemplateValues(
  ...
  const outputPath = provider.outputPath
    ? path.normalize(renderTemplate(provider.outputPath, {
      ...
      "desktop.deviceId": ports.getDesktopDeviceId(app),
...
async function resolveDesktopCapabilityInternal(
  ...
  if (provider.validateJwtDeviceId) {
    validateJwtDeviceId(text, options.ports.getDesktopDeviceId(app), validateExactDeviceId);
  }
  return {
    id: capabilityId,
    providerServiceId: service.id,
