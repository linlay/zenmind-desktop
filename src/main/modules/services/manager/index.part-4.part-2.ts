...
import { CORE_SERVICE_IDS, ServiceVerificationOptions, appendAgentContainerHubDesktopDeployArgs, appendAgentPlatformDesktopDeployArgs, appendAgentPlatformRuntimeResourceDeployArgs, appendAgentWebclientDesktopDeployArgs, appendDesktopConfigResetDeployArgs, appendIdentityCenterDesktopDeployArgs, copyDirectoryAssetToTempRoot, ensureDir, inFlightBuiltinInstalls, integrationPorts, isAssetNewerThanInstall, isDirectoryAssetPath, prepareServiceExecutionLayout } from "./index.part-1";
import { InstallBuiltinServiceOptions, buildVerificationResult, createBuiltinInstallKey, ensureDefaultConfig, getDependencyRunningVerificationTimeoutMs, getServiceState, hasVerifyRunningRequirements } from "./index.part-2";
import { applyEnvBindings, buildDesktopServiceCommandEnv, getServicePortForEnvSync } from "./index.part-3";
...
export async function ensureRequiredServiceHttpReachable(
  ...
  const authResult = authCapability
    ? await resolveCapability(app, authCapability, {
...
export async function collectDesktopCapabilityRequirementIssues(
  ...
  for (const requirement of requirements) {
    try {
      await applyDesktopCapabilityRequirement(app, service, layout, requirement, options, resolveCapability);
    } catch (error) {
