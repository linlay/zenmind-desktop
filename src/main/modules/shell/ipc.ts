...
import { getDesktopLogRoot, readDesktopLog, watchDesktopLog } from "../../support/logging/desktop";
import {
  createLogStreamSubscriptionRegistry,
  type LogStreamSubscriptionRegistry
} from "../../support/logging/subscriptions";
import {
...
export function registerShellIpcHandlers(ipcMain: Pick<IpcMain, "handle" | "on">, options: ShellIpcOptions) {
  ...
  ipcMain.handle("diagnostics.getTunnelDebugSnapshot", async () =>
    getTunnelDebugSnapshot(options.getTunnelHubRuntimeStatus ?? (() => ({ connected: false })))
  );
