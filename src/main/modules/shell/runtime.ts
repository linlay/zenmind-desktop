...
import { getArchiveExtensions } from "../../infrastructure/electron/platform-adapter";
import { resolveWebviewOpenDisposition, shouldDownloadUrlFromWebview } from "../web-surfaces";
import {
  getMainPreloadPath,
...
export function createAppShellRuntime(options: AppShellRuntimeOptions) {
  const state = {
    ...
    mainWindowSidebarTranslucencyEnabled: true,
  ...
  const mainWindowLifecycle = createMainWindowLifecycleController({
    ...
    clearWindow: (targetWindow) => {
      if (state.mainWindow === targetWindow) {
        state.mainWindow = null;
      }
    },
    isSidebarTranslucencyEnabled: () => state.mainWindowSidebarTranslucencyEnabled,
  ...
  async function showFileDialog(
    ...
    ownerWindow: BrowserWindow | null = state.mainWindow
  ) {
  ...
  function createWindow() {
    state.workPanelFullscreenActive = false;
    state.focusedWebviewDevToolsTargetId = null;
    state.mainWindow = new BrowserWindow(buildMainWindowOptions({
      platform: options.platform,
      preloadPath: getMainPreloadPath(options.mainProcessDir, options.platform),
    ...
    configureMainWindowWebContents<BrowserWindow, Electron.WebContents>(targetWindow, {
      ...
      onWebviewFocusChanged: (webContentsId, focused) => {
        if (focused) {
          state.focusedWebviewDevToolsTargetId = webContentsId;
        } else if (state.focusedWebviewDevToolsTargetId === webContentsId) {
          state.focusedWebviewDevToolsTargetId = null;
        }
      },
    ...
    configureMainWindowLifecycleEvents<BrowserWindow>(targetWindow, {
      ...
      clearWindow: (windowToClear) => {
        if (state.mainWindow === windowToClear) {
          state.mainWindow = null;
          state.workPanelFullscreenActive = false;
          state.focusedWebviewDevToolsTargetId = null;
        }
  ...
  function refreshMainWindowAppearance() {
    mainWindowLifecycle.applyAppearance(state.mainWindow);
  }
  ...
  function prepareQuitUi() {
    prepareQuitUiFromCleanup({
      ...
      keepVisibleWindow: state.mainWindow,
  ...
  return {
    getMainWindow: () => state.mainWindow,
    getFocusedWebviewDevToolsTargetId: () => state.focusedWebviewDevToolsTargetId,
    getWorkPanelFullscreenActive: () => state.workPanelFullscreenActive,
    setWorkPanelFullscreenActive: (active: boolean) => {
      state.workPanelFullscreenActive = active;
    },
