...
import { BrowserSurfaceLifecycleEvent, RegisteredSurface, RegisteredWebviewSurfaceTarget, SurfaceRegistrationDiagnostic, SurfaceRegistrationRejectionReason, SurfaceRegistrationValidation, diagnosticGuestWebContentsIds, guestTargetMatches, sanitizeSurfaceDiagnosticEnum, sanitizeSurfaceDiagnosticId } from "./browser-surface-registry.shared";
    ...
    if (input.surfaceLevel === "child") {
        ...
        if (input.parentSurfaceId) {
            ...
            if (canonicalParentSurfaceId !== input.parentSurfaceId ||
                ...
                (!context.resolveRegisteredSurface(canonicalParentSurfaceId) && context.workPanelDialogRegistrations.get(input.surfaceId)?.registrationId !== input.registrationId))
