...
import { cacheWebsiteFavicon } from "./websites/favicon-cache";
import {
  webappManager,
  WebappInstallError,
  WebappInstallPolicyError,
  WebappRuntimeRequiredError
} from "./webapps/manager";
...
export function listWebEntries(app: App, manager = webappManager) {
  const items = applyWebOrder(app, readWebItems(app, manager));
  return {
    ok: true,
    items,
...
export function registerWebIpcHandlers(ipcMain: any, options: WebIpcHandlerOptions) {
  ...
  ipcMain.handle("webs.list", async () => listWebEntries(app, webappManager));
  ...
  ipcMain.handle("webs.webapps.import", async () => {
    ...
    const currentItems = listWebEntries(app, webappManager).items;
    ...
    try {
      ...
      try {
        ...
        if (!executablePath || !path.isAbsolute(executablePath)) {
          ...
          return {
            ok: false,
            item: null,
            items: listWebEntries(app, webappManager).items,
            path: importPath,
      ...
      const nextItems = listWebEntries(app, webappManager).items;
