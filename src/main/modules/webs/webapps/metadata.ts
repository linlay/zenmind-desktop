...
import { getDesktopWebappsDataRoot, getServiceStateRoot } from "../../../infrastructure/filesystem/user-paths";
...
export function resolveWebappManagementMetadata(
  app: App,
  ...
  const marketRecord = requireWebsIntegrationPorts(ports).readInstalledRecords(app).find((record) =>
...
export function withWebappManagementMetadata(
  app: App,
  item: WebappEntry,
  ports?: WebsIntegrationPorts
): WebappEntry {
  return {
    ...item,
    ...resolveWebappManagementMetadata(app, item, ports)
  };
}
