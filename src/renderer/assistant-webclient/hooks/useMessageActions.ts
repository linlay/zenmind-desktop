...
export function useMessageActions() {
  ...
  useEffect(() => {
    const markNewConversationPending = () => {
      pendingNewConversationRef.current = true;
    };
    const clearNewConversationPending = (event: Event) => {
      const chatId = String((event as CustomEvent).detail?.chatId || '').trim();
      if (chatId) {
        pendingNewConversationRef.current = false;
      }
    };

    window.addEventListener('agent:start-new-conversation', markNewConversationPending);
    window.addEventListener('agent:load-chat', clearNewConversationPending);

    return () => {
      window.removeEventListener('agent:start-new-conversation', markNewConversationPending);
      window.removeEventListener('agent:load-chat', clearNewConversationPending);
    };
  }, []);
  ...
  const sendMessage = useCallback(
    async (
      ...
      const newConversationPending = pendingNewConversationRef.current;
      const targetChatId = resolveOutgoingChatId({
        preferredChatId,
        activeChatId: '',
        newConversationPending,
      });
      const isSameChat = isOutgoingChatSameAsActive({
        targetChatId,
        activeChatId: currentSessionChatId,
        newConversationPending,
      });
      ...
      const chatId = resolveOutgoingChatId({
        preferredChatId,
        activeChatId: stateRef.current.chatId,
        newConversationPending,
      });
