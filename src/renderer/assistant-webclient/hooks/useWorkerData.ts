...
export function shouldStartInitialWorkerRefresh(input: {
  ...
  if (input.hasStarted) {
    return Boolean(input.accessToken) && input.accessToken !== input.lastStartedToken;
  }
...
export function useWorkerData(input: {
  ...
  useEffect(() => {
    const currentToken = String(stateRef.current.accessToken || state.accessToken || '').trim();
    ...
    initialRefreshTokenRef.current = currentToken;
    setAccessToken(stateRef.current.accessToken);
    refreshWorkerData().catch(() => undefined);
  }, [refreshWorkerData, state.accessToken, state.transportMode, state.wsStatus, stateRef]);
