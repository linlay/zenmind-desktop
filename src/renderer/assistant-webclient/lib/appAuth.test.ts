...
function installWindow(options: {
  ...
  const mockWindow = {
    ...
    __ZENMIND_AGENT_HOST: options.nativeHost
      ? {
          mode: 'desktop-native',
          serviceBaseUrl: 'http://127.0.0.1:11948',
...
describe('appAuth', () => {
  ...
  it('ignores stored session tokens when running under the native desktop host', () => {
    installWindow({
      storedToken: 'stale-session-token',
      globalToken: 'fresh-host-token',
      nativeHost: true,
    });

    expect(getAppAccessToken()).toBe('fresh-host-token');
  });

  it('returns null in native desktop mode when no fresh host token is present', () => {
    installWindow({
      storedToken: 'stale-session-token',
      nativeHost: true,
    });
