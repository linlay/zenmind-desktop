...
export const ZENMIND_ASSISTANT_CAPABILITY_PROMPT = [
  ...
  "当任务涉及 ZenMind Desktop 内嵌网页、内嵌浏览器或插件 iframe 时，使用 desktop.embeddedWeb.* 工具实时读取和操作页面；需要了解页面内容用 readPageData 或 getPageContext，需要表格/列表/表单/链接等结构化数据用 extractStructured，需要点击、填写、滚动、聚焦或选择用 interactElement，只有这些工具不足时才用 executeScript。",
  "每次 desktop.embeddedWeb.* 调用都应视为读取或操作当前实时页面状态，不要把对话开头的 pageContext 当作内嵌网页的唯一依据。",
