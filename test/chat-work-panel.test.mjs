...
test("WorkPanel is AppShell-owned and keeps heterogeneous items mounted", () => {
  ...
  assert.match(appShell, /className=\{`main-chat-header-action main-chat-work-panel-toggle/u);
  ...
  assert.match(css, /\.main-chat-header-action\s*\{[^}]*width:\s*24px;[^}]*height:\s*24px;[^}]*border-radius:\s*var\(--control-radius-sm\)/su);
