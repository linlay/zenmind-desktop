...
test("release builds require an explicit product-bound Ed25519 public key", t => {
  ...
  fs.writeFileSync(file, JSON.stringify({ channel: "production", keys: [{ keyId: "main", productId: "cutej", channel: "production", publicKey: pair.publicKey.export({ type: "spki", format: "pem" }) }] }));
...
test("key generation refuses repository paths so private keys cannot enter build archives", t => {
  ...
  assert.throws(() => generateSigningKey(directory, "cutej", "production", "main", "test-passphrase-only"), /outside/i);
