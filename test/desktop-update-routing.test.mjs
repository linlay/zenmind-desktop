...
for (const platform of ['win32', 'darwin']) test(`configured ${platform} feed preserves its URL and pins signatures to the redirected release`, async t => {
  ...
  await fetchUpdateManifest(`https://example.com/${platform}/desktop-latest.json?channel=dev`, new AbortController().signal, platform);
  assert.equal(requests[0], `https://example.com/${platform}/desktop-latest.json?channel=dev`);
