...
test("task board route exposes native desktop api and page styles", () => {
  ...
  assert.match(taskBoardPage, /请输入任务描述。/);
  assert.match(taskBoardPage, /formCompact \? "高级模式" : "精简模式"/);
  ...
  assert.match(taskBoardPage, /"每天"/);
  assert.match(taskBoardPage, /"工作日"/);
  assert.match(taskBoardPage, /"每周"/);
