...
test("Kanban cards align issue meta, title, and hover actions with website", () => {
  ...
  assert.match(kanbanPage, /\{cardStatus\.updatedTime \? <span className="issue-card-status-time">\{cardStatus\.updatedTime\}<\/span> : null\}/);
  ...
  assert.match(kanbanPage, /className="issue-card-people-line"/);
  ...
  assert.match(kanbanStyles, /\.issue-card-row-top\s*\{[\s\S]{0,220}display:\s*grid;[\s\S]{0,220}grid-template-columns:\s*minmax\(0, 1fr\) max-content;/);
  assert.match(kanbanStyles, /\.issue-card-top-meta\s*\{[\s\S]{0,180}gap:\s*4px;[\s\S]{0,120}overflow:\s*hidden;/);
  assert.match(kanbanStyles, /\.kanban-priority-badge,[\s\S]{0,80}\.kanban-severity-badge\s*\{[\s\S]{0,180}height:\s*16px;/);
  assert.match(kanbanStyles, /\.kanban-title-text\s*\{[\s\S]{0,260}-webkit-line-clamp:\s*2;[\s\S]{0,180}font-weight:\s*400;/);
  assert.match(kanbanStyles, /\.issue-card-stage\s*\{[\s\S]{0,420}background:\s*var\(--issue-card-stage-bg, var\(--kanban-accent-soft\)\);[\s\S]{0,160}color:\s*var\(--issue-card-stage-color, var\(--kanban-accent\)\);/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-stage\s*\{[\s\S]{0,220}#eff6ff/);
  ...
  assert.match(kanbanStyles, /\.issue-card-people-line\s*\{/);
  assert.match(kanbanStyles, /\.issue-card\.has-agent::before\s*\{/);
...
test("Kanban route exposes native desktop api and page styles", () => {
  ...
  assert.doesNotMatch(kanbanPage, /issue-card-assignee-avatar/);
  ...
  assert.doesNotMatch(kanbanStyles, /\.issue-card-assignee-avatar\s*\{/);
