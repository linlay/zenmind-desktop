...
test("Kanban cards use stage-colored workflow rails and status-specific compact footers", () => {
  ...
  assert.match(kanbanPage, /function formatKanbanCompletionTime\(value: string \| null \| undefined, locale: SupportedLocale\)/);
  assert.match(kanbanPage, /function getIssueCardSignalPresentation\(\s*issue: KanbanIssue,\s*options:/);
  assert.match(kanbanPage, /issue\.status === "in_progress"[\s\S]{0,180}issue\.runStartedAt \|\| issue\.createdAt[\s\S]{0,180}kanban\.card\.runningFor/);
  assert.match(kanbanPage, /issue\.status === "in_review"[\s\S]{0,180}issue\.runStartedAt \|\| issue\.createdAt[\s\S]{0,220}kanban\.card\.runningFor/);
  assert.doesNotMatch(kanbanPage, /t\("kanban\.card\.waitingFor"/);
  assert.match(kanbanPage, /issue\.status === "completed" \|\| issue\.runState === "completed"[\s\S]{0,260}formatKanbanCompletionTime\(issue\.runFinishedAt \|\| issue\.updatedAt, options\.locale\)[\s\S]{0,180}kanban\.card\.completedAt/);
  assert.match(kanbanPage, /function getIssueCardOperationalStatePresentation/);
  assert.match(kanbanPage, /awaitingConfirmation && issue\.status === "in_progress"[\s\S]{0,180}kanban\.run\.awaitingApproval/);
  ...
  assert.match(kanbanPage, /const ISSUE_STAGE_COLOR_PALETTE = \[/);
  assert.match(kanbanPage, /function getIssueCardProgressPresentation\(/);
  assert.match(kanbanPage, /const stageProgress = Math\.min\(1, Math\.max\(0\.08,/);
  assert.match(kanbanPage, /const catalogStatusIndex = statusIndexById >= 0[\s\S]{0,160}statusIndexByKey >= 0[\s\S]{0,120}statusIndexByColumn/);
  ...
  assert.match(kanbanPage, /\{queueRank \? <span className="issue-card-queue-rank">\{queueRank\}<\/span> : null\}/);
  assert.match(kanbanPage, /className="issue-card-title-text"/);
  assert.match(kanbanPage, /const showDescription = issue\.status === "backlog" && Boolean\(descriptionPreview\)/);
  assert.match(kanbanPage, /\{showDescription \? \([\s\S]{0,160}className="issue-card-description"/);
  ...
  assert.match(kanbanPage, /issue\.status === "backlog" \? \([\s\S]{0,900}issue\.status === "todo" \? \([\s\S]{0,900}issue\.status === "in_progress" \|\| issue\.status === "in_review" \? \(/);
  assert.match(kanbanPage, /issue\.status === "in_progress" \|\| issue\.status === "in_review"[\s\S]{0,500}\{timingSignal\}[\s\S]{0,220}\{due\}[\s\S]{0,220}\{actionSlot\(stateSignal\)\}/);
  assert.match(kanbanPage, /<div className="issue-card-footer-row">\{timingSignal\}<\/div>[\s\S]{0,160}<div className="issue-card-footer-row">\{people\}\{actionSlot\(\)\}<\/div>/);
  assert.match(kanbanPage, /const actionSlot = \(signal: ReactNode = null\) => \([\s\S]{0,180}issue-card-footer-end has-\$\{actionCount\}-actions/);
  ...
  assert.match(kanbanStyles, /\.issue-card-description\s*\{[\s\S]{0,180}-webkit-line-clamp:\s*2;/);
