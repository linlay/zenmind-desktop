...
test("Kanban route exposes native desktop api and page styles", () => {
  ...
  assert.match(globalStyles, /--kanban-column-gap:\s*8px;/);
  ...
  assert.match(globalStyles, /\.kanban-column\s*\{[\s\S]{0,320}border:\s*1px solid var\(--kanban-column-border\);[\s\S]{0,180}border-radius:\s*12px;[\s\S]{0,220}box-shadow:\s*none;/);
  assert.doesNotMatch(globalStyles, /\.kanban-column \+ \.kanban-column\s*\{/);
