...
test("settings page configures desktop helper default agent separately from desktop pet", () => {
  ...
  assert.doesNotMatch(settingsPage, /taskBoard\.cloud\.deviceAliasPlaceholder/);
...
test("task board route exposes native desktop api and page styles", () => {
  ...
  assert.match(mainIpcRegister, /registerTaskBoardIpcHandlers\(ipcMain,/);
  ...
  assert.match(assistantRuntime, /createTaskBoardRuntime/);
  assert.match(assistantRuntime, /state\.taskBoardRuntime\?\.sendAssistantEvent\(event\)/);
  ...
  assert.match(globalStyles, /\.task-board-chat-modal \.embedded-surface-page\s*\{/);
});
...
test("embedded H5 routes keep a thin global window drag lane", () => {
  ...
  assert.doesNotMatch(externalBrowserChromeRule, /app-region:\s*drag;/);
  assert.doesNotMatch(externalBrowserChromeRule, /-webkit-app-region:\s*drag;/);
