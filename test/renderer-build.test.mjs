...
test("settings route moves section navigation into the app sidebar and uses section subroutes", () => {
  ...
  assert.match(settingsStyles, /\.settings-page-single\[data-settings-section="control"\]\s*\{[\s\S]*?padding-top:\s*0;/);
...
test("settings page configures desktop helper default agent separately from desktop pet", () => {
  ...
  assert.doesNotMatch(sectionHeaderActionBody, /case "tunnelHub"/);
  assert.match(settingsPage, /const shouldShowSettingsPageHead = activeSection !== "control";/);
...
test("settings page keeps Kanban, Control, and Tunnel Hub separate", () => {
  ...
  assert.match(tunnelHubCaseBody, /settings\.tunnelHub\.panelAria[\s\S]*settings\.tunnelHub\.connectionTitle[\s\S]*handleToggleTunnelHubEnabled[\s\S]*handleSaveTunnelHubSettings[\s\S]*settings\.tunnelHub\.relayUrl/);
...
test("assistant navigation agents are exposed through dedicated ipc without changing pet agents", () => {
  ...
  assert.match(appSidebar, /value=\{createProjectDialog\.name\}[\s\S]*?name:\s*event\.target\.value/);
  assert.doesNotMatch(appSidebar, /<input\s+value=\{createProjectDialog\.name\}\s+disabled\s*\/>/);
  ...
  assert.match(appSidebar, /value="local"[\s\S]*?t\("sidebar\.project\.localVectorStore"\)/);
  assert.match(appSidebar, /value="remote"[\s\S]*?t\("sidebar\.project\.remoteVectorStore"\)/);
  assert.match(appSidebar, /error:\s*t\("sidebar\.project\.kbaseNotImplemented"\)/);
  const kbaseGuardIndex = indexOfRequired(appSidebar, 'if (dialog.projectType === "kbase")');
  const createCoderProjectCallIndex = indexOfRequired(appSidebar, "window.electronAPI.assistant.createCoderProject(createInput)");
  assert.ok(kbaseGuardIndex < createCoderProjectCallIndex);
