...
test("assistant launcher sits beside the sidebar collapse button", () => {
  ...
  assert.match(appShell, /return createAgentWebclientCopilotPath\(agentKey, params\)/);
  assert.match(appShell, /decodeRoutePathSegment\(rawAgentKey\) \?\? ""/);
...
test("fixed sidebar tool trigger exposes account state and Help affordance", () => {
  ...
  const triggerMarkup = sidebarSource.slice(
    indexOfRequired(sidebarSource, "{!shouldRenderDesktopSsoTrigger ? ("),
    indexOfRequired(sidebarSource, '{getCollapsedSidebarLabel(t("nav.settings"))}')
  );

  assert.match(
    sidebarSource,
    /function isFixedToolRouteActive\(targetPath: string\) \{[\s\S]*?targetPathname === "\/agents"[\s\S]*?displayCurrentPathname\.startsWith\(`\$\{targetPathname\}\/`\)[\s\S]*?pendingPathname\.startsWith\(`\$\{targetPathname\}\/`\)[\s\S]*?displayCurrentPathname === targetPathname[\s\S]*?pendingPathname === targetPathname/
  );
  assert.match(
    sidebarSource,
...
test("sidebar renders Kanban and section groups above the fixed tool menu", () => {
  ...
  assert.match(appShell, /const agentKey = decodeRoutePathSegment\(encodedAgentKey\)/);
  assert.match(appShell, /embedPath:\s*createAgentWebclientManagementPath\(agentKey, search\)/);
  ...
  assert.match(appShell, /function resolveAgentManagementWebclientRoute\(pathname: string, search: string\)[\s\S]*?mode:\s*"embedded"/);
  assert.match(appShell, /const businessSearch = createAgentWebclientBusinessSearch\(search\)/);
...
test("assistant sidebar uses merged activity agents when available", () => {
  ...
  assert.match(appShell, /const nextItems = normalizeAssistantNavAgents\(resolveAssistantNavDisplayItems\(nextResult\)\);/);
...
test("Kanban cards use a three-section hierarchy with granular status and stable actions", () => {
  ...
  assert.match(contracts, /runStartedAt\?: string \| null/);
  assert.match(contracts, /runFinishedAt\?: string \| null/);
  assert.match(contracts, /workerType\?: "human" \| "agent" \| null/);
  assert.match(kanbanPage, /function formatIssueUpdatedTime\(updatedAt: string, currentDate = new Date\(\)\)/);
  ...
  assert.match(kanbanPage, /function formatKanbanCompactDuration\(value: string \| null \| undefined, now: Date\)/);
  assert.match(kanbanPage, /totalMinutes < 1[\s\S]{0,80}return "<1m"/);
  assert.match(kanbanPage, /return hours > 0 \? `\$\{days\}d \$\{hours\}h` : `\$\{days\}d`/);
  assert.match(kanbanPage, /return minutes > 0 \? `\$\{hours\}h \$\{minutes\}m` : `\$\{hours\}h`/);
  assert.match(kanbanPage, /timestamp > nowTimestamp/);
  assert.match(kanbanPage, /function getIssueCardSignalPresentation\(\s*issue: KanbanIssue,\s*options:/);
  assert.match(kanbanPage, /issue\.status === "todo"[\s\S]{0,220}formatKanbanSortNumber\(options\.sortIndex, issue\.position\)[\s\S]{0,120}icon: "order"/);
  assert.doesNotMatch(kanbanPage, /automationCountdown \|\| formatKanbanSortNumber/);
  ...
  assert.match(kanbanPage, /issue\.status === "in_progress" && \(issue\.runState === "running" \|\| Boolean\(issue\.runId\)\)[\s\S]{0,360}issue\.runStartedAt[\s\S]{0,220}kanban\.card\.runningFor/);
  assert.match(kanbanPage, /issue\.status === "completed" \|\| issue\.runState === "completed"[\s\S]{0,360}issue\.runFinishedAt[\s\S]{0,220}kanban\.card\.completedAgo/);
  assert.match(kanbanPage, /issue\.status === "in_review"[\s\S]{0,320}kanban\.card\.waitingFor/);
  assert.match(kanbanPage, /function getIssueGranularStatusLabel\(issue: KanbanIssue, t: TranslateFunction\)/);
  assert.match(kanbanPage, /"inprogress"[\s\S]{0,160}return columnLabels\.has\(normalizedLabel\) \? "" : label/);
  assert.match(kanbanPage, /const granularStatusLabel = getIssueGranularStatusLabel\(issue, t\)/);
  assert.match(kanbanPage, /function getIssueGranularStatusTone\(issue: KanbanIssue\)/);
  assert.doesNotMatch(kanbanPage, /label: t\(STATUS_META\[issue\.status\]\.labelKey\)/);
  assert.match(kanbanPage, /const cardSignal = getIssueCardSignalPresentation\(issue, \{[\s\S]{0,120}awaitingConfirmation[\s\S]{0,120}sortIndex/);
  ...
  assert.match(kanbanPage, /className="issue-card-section issue-card-context"/);
  assert.match(kanbanPage, /className="issue-card-context-line is-primary"/);
  assert.match(kanbanPage, /className="issue-card-context-line is-taxonomy"/);
  assert.match(kanbanPage, /<ApartmentOutlined aria-hidden="true" \/>/);
  assert.match(kanbanPage, /<TagOutlined aria-hidden="true" \/>/);
  assert.match(kanbanPage, /className="issue-card-taxonomy-item is-stage"/);
  assert.match(kanbanPage, /className=\{`issue-card-taxonomy-item is-status is-\$\{granularStatusTone\}`\}/);
  assert.match(kanbanPage, /<section className="issue-card-section issue-card-title-block" title=\{issue\.title\}>/);
  assert.doesNotMatch(kanbanPage, /<strong title=\{description \|\| issue\.title\}>/);
  assert.doesNotMatch(kanbanPage, /descriptionPreview/);
  assert.doesNotMatch(kanbanPage, /issue-card-description/);
  ...
  assert.match(kanbanPage, /className=\{`issue-card-signal is-\$\{cardSignal\.tone\}`\}/);
  assert.match(kanbanPage, /<IssueCardSignalIcon kind=\{cardSignal\.icon\} \/>/);
  assert.match(kanbanPage, /function IssueCardSignalIcon/);
  assert.doesNotMatch(kanbanPage, /kanban-run-dot/);
  assert.doesNotMatch(kanbanPage, /issue-card-status-dot/);
  assert.doesNotMatch(kanbanPage, /className="issue-card-footer-signals"/);
  assert.doesNotMatch(kanbanPage, /className="issue-card-footer-signal"/);
  assert.match(kanbanPage, /className="issue-card-footer-badges"/);
  assert.match(kanbanPage, /className="issue-card-footer-end"/);
  assert.match(kanbanPage, /className="issue-card-people"/);
  assert.match(kanbanPage, /className="issue-card-person is-assignee"/);
  assert.match(kanbanPage, /issue-card-person is-worker/);
  ...
  assert.doesNotMatch(kanbanPage, /title=\{automationLabel\}/);
  assert.match(kanbanPage, /const shouldShowAssignee = peopleLine\.assigneeLabel !== unassignedLabel \|\| canEditKanbanIssueBody\(issue\)/);
  assert.match(kanbanPage, /const shouldShowPeople = display\.assignee && \(shouldShowAssignee \|\| Boolean\(peopleLine\.worker\)\)/);
  assert.doesNotMatch(kanbanPage, /description: t\("kanban\.display\.description"\)/);
  ...
  assert.match(kanbanStyles, /\.issue-card\s*\{[\s\S]{0,260}min-height:\s*206px;/);
  assert.match(kanbanStyles, /\.issue-card-context\s*\{[\s\S]{0,180}min-height:\s*78px;/);
  assert.match(kanbanStyles, /\.issue-card-context-line\.is-taxonomy\s*\{[\s\S]{0,140}margin-top:\s*10px;/);
  assert.match(kanbanStyles, /\.issue-card-taxonomy-item\.is-status\s*\{[\s\S]{0,120}justify-content:\s*flex-end;/);
  assert.match(kanbanStyles, /\.issue-card-title-block > span\s*\{[\s\S]{0,260}font-weight:\s*400;[\s\S]{0,260}-webkit-line-clamp:\s*2;/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-description\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-status-dot\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-footer-signals\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-footer-signal\s*\{/);
  assert.match(kanbanStyles, /\.issue-card-footer-badge\s*\{[\s\S]{0,220}border:\s*none;[\s\S]{0,120}background:\s*transparent;/);
  assert.match(kanbanStyles, /\.issue-card-foot\s*\{[\s\S]{0,220}display:\s*flex;[\s\S]{0,120}flex-wrap:\s*nowrap;/);
  ...
  assert.match(kanbanStyles, /\.issue-card:hover \.issue-card-people,[\s\S]{0,80}\.issue-card:focus-within \.issue-card-people\s*\{[\s\S]{0,100}opacity:\s*0;/);
  assert.match(kanbanStyles, /\.issue-card-action\s*\{[\s\S]{0,180}width:\s*24px;[\s\S]{0,180}height:\s*24px;/);
  ...
  assert.match(zhCN, /"kanban\.card\.status": "当前状态：\{value\}"/);
  assert.match(zhCN, /"kanban\.card\.runningFor": "运行 \{duration\}"/);
  assert.match(zhCN, /"kanban\.card\.waitingFor": "等待 \{duration\}"/);
  assert.match(zhCN, /"kanban\.card\.completedAgo": "\{duration\} 前完成"/);
  ...
  assert.match(enUS, /"kanban\.card\.status": "Current status: \{value\}"/);
  assert.match(enUS, /"kanban\.card\.completedAgo": "Completed \{duration\} ago"/);
...
test("Kanban todo cards keep their queue order while the shared timer refreshes run context", () => {
  ...
  assert.doesNotMatch(kanbanPage, /function formatKanbanAutomationCountdown/);
  assert.match(kanbanPage, /issue\.status === "todo"[\s\S]{0,180}label: formatKanbanSortNumber\(options\.sortIndex, issue\.position\)/);
...
test("Kanban route exposes native desktop api and page styles", () => {
  ...
  assert.match(kanbanPage, /if \(!chatId\) \{[\s\S]{0,100}return createAgentWebclientAgentPath\(agentKey\)/);
  ...
  assert.doesNotMatch(kanbanPage, /issue-card-footer-signal/);
  ...
  assert.match(kanbanPage, /<footer className="issue-card-section issue-card-foot">/);
  ...
  assert.match(kanbanPage, /className="kanban-toolbar-start"[\s\S]{0,180}className="kanban-page-title"/);
  ...
  assert.match(kanbanStyles, /\.issue-card-title-block > span\s*\{[\s\S]{0,260}-webkit-line-clamp:\s*2;/);
  assert.match(kanbanStyles, /\.issue-card-title-block > span\s*\{[\s\S]{0,320}font-weight:\s*400;/);
  ...
  assert.match(globalStyles, /\.issue-card-signal,\s*\.issue-card-taxonomy-item\s*\{[\s\S]{0,220}overflow:\s*hidden;/);
...
test("webapps expose desktop api and start from webs sidebar route", () => {
  ...
  assert.match(contracts, /webapps:\s*\{[\s\S]*getPublishStatus: \(id: string\) => Promise<WebappPublishStatusResult>/);
  ...
  assert.match(preload, /getPublishStatus: \(id: string\) => ipcRenderer\.invoke\("webs\.webapps\.getPublishStatus", id\)/);
  ...
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.list"[\s\S]*webappManager\.listResult\(app\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.export"[\s\S]*webappManager\.exportArchive\(app, item\.id, saveResult\.filePath\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.update"[\s\S]*webappManager\.update\(app, id, input\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.uninstall"[\s\S]*webappManager\.remove\(app, id\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.checkRuntime"[\s\S]*webappManager\.runtime\.checkRuntime\(app, id\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.getPublishStatus"[\s\S]*getWebappPublishStatus\(app, id\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.start"[\s\S]*webappManager\.runtime\.start\(app, id\)/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.openWindow"/);
  assert.match(webHandlers, /ipcMain\.handle\("webs\.webapps\.stop"[\s\S]*webappManager\.runtime\.stop\(app, id\)/);
  ...
  assert.match(shutdownRunner, /stopWebapps:\s*\(app\) => webappManager\.runtime\.stopAll\(app\)/);
...
test("assistant navigation agents are exposed through dedicated ipc without changing pet agents", () => {
  ...
  assert.match(appSidebar, /return createAgentWebclientManagementPath\(agent\.agentKey\);/);
...
test("desktop global search contract is wired across main preload renderer and help", () => {
  ...
  assert.match(contracts, /onGlobalSearchShortcut: \(listener: DesktopGlobalSearchShortcutListener\) => \(\) => void/);
  ...
  assert.match(preload, /ipcRenderer\.on\("app\.globalSearchShortcut"/);
  ...
  assert.match(platformAdapter, /export function resolveGlobalSearchCommandShortcut/);
  ...
  assert.match(windowManager, /app\.globalSearchShortcut/);
  ...
  assert.match(overlay, /const newChatAgentKey = currentAgentKey \|\| props\.defaultChatAgentKey\.trim\(\)/);
  ...
  assert.match(overlay, /optionOnly \? <OptionShortcutIcon \/> : <CommandShortcutIcon \/>/);
  assert.match(overlay, /function CommandShortcutIcon\(\)[\s\S]*?<svg className="desktop-global-search-shortcut-icon"/);
  assert.match(overlay, /function OptionShortcutIcon\(\)[\s\S]*?<svg className="desktop-global-search-shortcut-icon"/);
  assert.match(overlay, /shortcut\.kind === "agent"[\s\S]*?`Alt\+\$\{suffix\}`/);
  ...
  assert.match(rows, /awaitingMode:\s*chat\.awaitingMode/);
  assert.match(rows, /awaitingMode:\s*localRow\?\.awaitingMode/);
  assert.match(rows, /row\.isUnread/);
  assert.match(rows, /attention:[\s\S]*?\.slice\(0, 10\),[\s\S]*?agents:[\s\S]*?\.slice\(0, 10\)/);
  assert.match(assistantNavigation, /export function getAssistantAwaitingStatusKey/);
  assert.match(i18nEn, /"desktop\.globalSearch\.group\.awaiting": "Awaiting"/);
  assert.doesNotMatch(i18nEn, /desktop\.globalSearch\.status\.awaiting/);
...
test("desktop action bridge exposes localhost api and renderer action providers", () => {
  ...
  assert.match(actionCatalog, /desktop\.webapp\.install/);
  assert.match(actionCatalog, /desktop\.webapp\.checkRuntime/);
  assert.match(actionCatalog, /desktop\.webapp\.getPublishStatus/);
  assert.match(actionCatalog, /desktop\.webapp\.uninstall/);
  assert.doesNotMatch(actionCatalog, /desktop\.webapp\.(?:installAndOpen|selectDirectory|checkPrerequisites|getPublishInfo)/);
