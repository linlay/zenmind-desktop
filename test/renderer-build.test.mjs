...
test("settings page configures desktop helper default agent separately from desktop pet", () => {
  ...
  assert.match(settingsPage, /settings\.general\.desktopInfoTitle/);
...
test("Kanban cards align issue meta, title, and hover actions with website", () => {
  ...
  assert.match(kanbanPage, /<span className="issue-card-chip issue-card-origin"[\s\S]{0,120}title=\{issueOrigin\.title\}[\s\S]{0,120}\{issueOrigin\.projectLabel\}/);
  assert.match(kanbanPage, /<IssuePriorityBadge priority=\{issue\.priority\} t=\{t\} \/>/);
  assert.match(kanbanPage, /<IssueSeverityBadge severity=\{severity\} t=\{t\} \/>/);
  assert.match(kanbanPage, /className="kanban-priority-text">\{shortLabel\}<\/span>/);
  assert.match(kanbanPage, /className="kanban-severity-text">\{shortLabel\}<\/span>/);
  assert.match(kanbanPage, /\{stageLabel \? \([\s\S]{0,220}<span className="issue-card-chip issue-card-stage"[\s\S]{0,180}\{stageLabel\}/);
  assert.match(kanbanPage, /<span className="kanban-title-body">\{issue\.title\}<\/span>/);
  ...
  assert.match(kanbanPage, /\{cardStatus\.tone !== "backlog" && cardStatus\.tone !== "todo" \? <span className="kanban-run-dot" aria-hidden="true" \/> : null\}/);
  assert.match(kanbanPage, /<span className="issue-card-status-label">\{cardStatus\.label\}<\/span>/);
  assert.match(kanbanPage, /<span className="issue-card-status-time">\{cardStatus\.updatedTime\}<\/span>/);
  assert.match(kanbanPage, /title=\{automationLabel\}/);
  assert.match(kanbanPage, /className="kanban-automation-label">\{automationLabel\}<\/span>/);
  ...
  assert.match(kanbanStyles, /\.issue-card-row-top\s*\{[\s\S]{0,160}min-height:\s*16px;[\s\S]{0,80}height:\s*auto;/);
  assert.match(kanbanStyles, /\.issue-card-top-meta\s*\{[\s\S]{0,180}gap:\s*4px;[\s\S]{0,120}overflow:\s*hidden;/);
  assert.match(kanbanStyles, /\.kanban-priority-badge,[\s\S]{0,80}\.kanban-severity-badge\s*\{[\s\S]{0,180}height:\s*15px;/);
  assert.match(kanbanStyles, /\.kanban-title-text\s*\{[\s\S]{0,260}-webkit-line-clamp:\s*2;[\s\S]{0,180}font-weight:\s*400;/);
  assert.match(kanbanStyles, /\.issue-card-stage\s*\{[\s\S]{0,420}line-height:\s*16px;/);
  assert.match(kanbanStyles, /\.issue-card-stage\s*\{[\s\S]{0,180}background:\s*#eff6ff;[\s\S]{0,120}color:\s*#2563eb;/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-stage\s*\{[\s\S]{0,220}--kanban-accent/);
  assert.match(kanbanStyles, /\.issue-card-description\s*\{[\s\S]{0,220}-webkit-line-clamp:\s*2;/);
  assert.match(kanbanStyles, /\.issue-card-status\s*\{[\s\S]{0,180}max-width:[\s\S]{0,180}height:\s*auto;/);
  assert.match(kanbanStyles, /\.issue-card-status\.is-succeeded[\s\S]{0,160}#15803d/);
  assert.match(kanbanStyles, /\.issue-card-status\.is-failed[\s\S]{0,160}#b91c1c/);
  assert.match(kanbanStyles, /\.issue-card-status\.is-cancelled[\s\S]{0,160}#475569/);
  assert.match(kanbanStyles, /\.issue-card-status\.is-awaiting[\s\S]{0,160}#b45309/);
  assert.match(kanbanStyles, /\.issue-card-status\.is-running[\s\S]{0,160}#b45309/);
  const kanbanStatusRules = kanbanStyles.match(/[^{}]*\.issue-card-status\.is-[^{}]*\{[^{}]*\}/g) ?? [];
  assert.notEqual(kanbanStatusRules.length, 0);
  for (const rule of kanbanStatusRules.filter(
    (rule) => !rule.includes(".kanban-run-dot")
  )) {
    assert.doesNotMatch(rule, /background:/);
  }
  assert.match(kanbanStyles, /\.kanban-automation-label\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card::before/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card\.is-[^{]+::before/);
  assert.match(kanbanStyles, /\.issue-card-actions\s*\{[\s\S]{0,180}opacity:\s*0;[\s\S]{0,80}pointer-events:\s*none;/);
  ...
  assert.match(kanbanStyles, /\.issue-card-origin/);
...
test("Kanban cloud popover resyncs and toolbar filters by project tree", () => {
  ...
  assert.match(kanbanPage, /function flattenKanbanProjectTree\(projects: KanbanProject\[\]\)/);
...
test("Kanban route exposes native desktop api and page styles", () => {
  ...
  assert.match(kanbanPage, /function openAssistantIssueChat/);
  ...
  assert.match(kanbanPage, /function openEditModal\(issue: KanbanIssue\)[\s\S]{0,260}setFormCompact\(canEditKanbanIssueBody\(issue\) \? !hasIssueAutomation\(issue\) : false\);/);
  ...
  assert.match(kanbanPage, /kanban-automation-badge/);
  ...
  assert.match(kanbanPage, /kanban-attachment-badge/);
  ...
  assert.match(kanbanPage, /function truncateKanbanAssigneeName\(name: string\)[\s\S]{0,120}Array\.from\(name\.trim\(\)\)\.slice\(0, 4\)\.join\(""\)/);
  assert.match(kanbanPage, /const visibleAssigneeName = getVisibleAssigneeName\(issue, agents\)/);
  assert.match(kanbanPage, /return truncateKanbanAssigneeName\(visibleAssigneeName\);/);
  ...
  assert.match(kanbanPage, /const assigneeAgent = getAssigneeAgent\(issue, agents\)/);
  assert.match(kanbanPage, /const assigneeIcon = hasKanbanAgentIcon\(assigneeAgent\?\.icon\) \? assigneeAgent\?\.icon : undefined/);
  assert.match(kanbanPage, /function getIssueCardAssigneeAvatarLabel\(name: string\)/);
  assert.doesNotMatch(kanbanPage, /function KanbanAssigneeIcon/);
  assert.match(kanbanPage, /className=\{`issue-card-assignee-avatar\$\{assigneeIcon \? " has-icon" : ""\}`\}/);
  assert.match(kanbanPage, /assigneeIcon \? \([\s\S]{0,180}<AgentIcon[\s\S]{0,180}icon=\{assigneeIcon\}[\s\S]{0,180}className="issue-card-assignee-icon"[\s\S]{0,180}size=\{18\}/);
  assert.match(kanbanPage, /\) : \([\s\S]{0,120}className="issue-card-assignee-avatar-label"[\s\S]{0,120}getIssueCardAssigneeAvatarLabel\(visibleAssigneeName\)/);
  assert.doesNotMatch(kanbanPage, /issue-card-assignee-avatar-label[\s\S]{0,220}assigneeIcon \? \(/);
  assert.match(kanbanPage, /className="issue-card-assignee-avatar-label"/);
  assert.match(kanbanPage, /getIssueCardAssigneeAvatarLabel\(visibleAssigneeName\)/);
  ...
  assert.match(kanbanStyles, /\.issue-card-assignee-icon\s*\{[\s\S]{0,180}width:\s*16px;[\s\S]{0,180}height:\s*16px;/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-assignee-icon::before/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-assignee-icon::after/);
  assert.doesNotMatch(globalStyles, /#fde047 0%, #facc15 48%, #d69e13/);
  assert.match(kanbanStyles, /\.issue-card-assignee-avatar\s*\{/);
  assert.match(kanbanStyles, /\.issue-card-assignee-avatar-label\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-assignee-avatar\.has-icon \.issue-card-assignee-avatar-label/);
  ...
  assert.match(kanbanStyles, /\.kanban-title-text\s*\{[\s\S]{0,260}-webkit-line-clamp:\s*2;/);
  assert.match(kanbanStyles, /\.kanban-title-text\s*\{[\s\S]{0,320}font-weight:\s*400;/);
  ...
  assert.match(globalStyles, /\.issue-card-actions\s*\{[\s\S]{0,220}opacity:\s*0;[\s\S]{0,120}pointer-events:\s*none;/);
