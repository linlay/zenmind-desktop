...
test("chat headers expose sorting and new chat while the chat menu keeps sharing", () => {
  const sidebarSource = readSourceFile(
    "src",
    "renderer",
    "app-shell",
...
test("Kanban cards match the Website hierarchy and date-only contract", () => {
  ...
  assert.match(kanbanPage, /className="issue-card-type-corner"/);
  assert.match(kanbanPage, /className="issue-card-type-corner-icon"/);
  ...
  assert.match(kanbanPage, /className="issue-card-queue-rank"/);
  ...
  assert.doesNotMatch(kanbanPage, /issue-card-priority-divider/);
  assert.match(kanbanStyles, /\.issue-card-priority-importance\.is-severity-critical/);
  assert.match(kanbanStyles, /\.issue-card-priority-importance\.is-severity-unset/);

  assert.match(kanbanPage, /function getIssueCardDuePresentation[\s\S]{0,1000}Date\.UTC\(year, month - 1, day\)/);
  assert.match(kanbanPage, /timeZone: "UTC"/);
  ...
  assert.match(kanbanPage, /type SearchFilterMenuKind = "issueType" \| "priority" \| "severity"/);
  assert.match(kanbanPage, /issue\.priority \?\? "unset"/);
  assert.match(kanbanPage, /issueSeverity \?\? "unset"/);
  assert.match(kanbanPage, /issue\.issueTypeKey \?\? issue\.typeId[\s\S]*issue\.reporterId[\s\S]*issue\.projectPath[\s\S]*\.\.\.issue\.componentKeys/);

  assert.match(kanbanStyles, /--kanban-content-min-width:\s*1080px/);
  assert.match(kanbanStyles, /\.issue-card\s*\{[\s\S]{0,360}width:\s*100%;[\s\S]{0,180}min-width:\s*200px/);
  ...
  assert.match(zhCN, /"kanban\.card\.dueDateMissing"/);
  assert.match(zhCN, /"kanban\.card\.dueRisk"/);
  assert.match(enUS, /"kanban\.card\.updatedAt"/);
  assert.match(enUS, /"kanban\.card\.version"/);
});
...
test("Kanban todo cards keep their queue order while the shared timer refreshes run context", () => {
  ...
  assert.match(kanbanPage, /const queueRank = issue\.status === "todo" \? formatKanbanSortNumber\(sortIndex, issue\.position\) : ""/);
  assert.match(kanbanPage, /\{queueRank \? <span className="issue-card-queue-rank"/);
...
test("Kanban route exposes native desktop api and page styles", () => {
  ...
  assert.match(preload, /ipcRenderer\.invoke\("kanban\.claimIssue", issueId\)/);
  ...
  assert.doesNotMatch(kanbanPage, /ServiceWebviewSurface|chatModalRequest|buildKanbanChatEmbedPath/);
  assert.match(kanbanPage, /const navigate = useNavigate\(\)/);
  assert.match(kanbanPage, /kanbanApi\.claimIssue\(issue\.id\)/);
  assert.match(kanbanPage, /kanbanApi\.runIssue\(\{ issueId: issue\.id, agentKey \}\)/);
  ...
  assert.match(kanbanPage, /const peopleLine = getIssueCardPeoplePresentation\(issue, agents, cloudDetails\.users, t\)/);
  ...
  assert.doesNotMatch(kanbanPage, /className="issue-card-action"/);
  ...
  assert.match(kanbanStyles, /\.issue-card-title\s*\{[\s\S]{0,180}max-height:\s*36px;[\s\S]{0,100}overflow:\s*hidden;/);
  ...
  assert.doesNotMatch(globalStyles, /\.issue-card-actions\s*\{/);
  assert.doesNotMatch(globalStyles, /\.issue-card:hover \.issue-card-actions/);
  ...
  assert.doesNotMatch(globalStyles, /\.issue-card-action\s*\{/);
...
test("desktop global search contract is wired across main preload renderer and help", () => {
  ...
  assert.match(appShell, /onOpenHistory=\{\(\) => openChatHistoryDialog\(\)\}/);
  ...
  assert.match(overlay, /row\.actionId === "history"[\s\S]*?<ClockCircleOutlined \/>/);
...
test("embedded H5 routes keep a thin global window drag lane", () => {
  ...
  assert.match(globalStyles, /\.app-shell\.has-embedded-surface\s*\{[^}]*--app-window-drag-height:\s*12px;/);
  assert.match(globalStyles, /\.app-shell\.is-mac-platform\s*\{[^}]*--app-window-drag-height:\s*12px;/);
  ...
  assert.match(
    globalStyles,
    /\.app-window-controls-layer\s*\{[^}]*z-index:\s*1001;[^}]*pointer-events:\s*none;/
  );
  assert.match(
    globalStyles,
  ...
  assert.doesNotMatch(
    globalStyles,
    /\.app-shell\.is-mac-platform\.has-chat-work-panel\s*\{[^}]*--app-window-drag-right:/
  );
  assert.doesNotMatch(
    globalStyles,
...
test("desktop pet message reaction collapses to an unread badge without an expand button", () => {
  ...
  assert.match(globalStyles, /\.desktop-pet-message-card \+ \.desktop-pet-message-card\s*\{[\s\S]{0,120}border-top:\s*1px solid var\(--pet-line\);/);
  assert.match(globalStyles, /\.desktop-pet-message-title-line strong\s*\{[\s\S]{0,220}font-size:\s*13px;[\s\S]{0,80}font-weight:\s*550;/);
  assert.match(globalStyles, /\.desktop-pet-message-preview\s*\{[\s\S]{0,220}font-size:\s*12px;[\s\S]{0,80}font-weight:\s*400;/);
  ...
  assert.match(globalStyles, /\.desktop-pet-message-reply\s*\{[\s\S]{0,100}grid-column:\s*2;[\s\S]{0,80}grid-row:\s*1;/);
  assert.match(globalStyles, /\.desktop-pet-message-reply\s*\{[^}]*?background:\s*var\(--pet-accent-soft\);/);
  ...
  assert.match(globalStyles, /\.desktop-pet-root\.is-panel-window\.has-bubble \.desktop-pet-status-panel\s*\{[\s\S]{0,260}box-shadow:[\s\S]{0,120}0 4px 14px rgba\(15,\s*23,\s*42,\s*0\.1\)/);
  assert.match(globalStyles, /\.desktop-pet-status-panel\s*\{[^}]*?gap:\s*8px;[^}]*?padding:\s*12px;/);
  assert.match(globalStyles, /\.desktop-pet-status-panel \.desktop-pet-task-head\s*\{[\s\S]{0,120}grid-template-columns:\s*minmax\(0,\s*1fr\) 32px;[\s\S]{0,80}min-height:\s*26px;/);
  ...
  assert.match(globalStyles, /\.desktop-pet-message-card\.is-replying\s*\{[\s\S]{0,180}min-height:\s*150px;/);
  assert.match(globalStyles, /\.desktop-pet-message-card\.is-replying\s*\{[\s\S]{0,180}padding:\s*10px 8px 14px;/);
  assert.match(globalStyles, /\.desktop-pet-message-reply-box\s*\{[^}]*?gap:\s*12px 6px;[^}]*?margin-top:\s*12px;/);
  assert.match(globalStyles, /\.desktop-pet-message-reply-input\s*\{[\s\S]{0,100}grid-column:\s*1 \/ -1;/);
