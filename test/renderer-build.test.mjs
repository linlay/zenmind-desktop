...
test("Kanban cards use a three-section hierarchy with granular status and stable actions", () => {
  ...
  assert.match(contracts, /dueAt\?: EpochMilliseconds \| null/);
  ...
  assert.match(kanbanPage, /function getIssueCardStatusPresentation\(issue: KanbanIssue, t: TranslateFunction\)/);
  assert.match(kanbanPage, /label: granularLabel \|\| t\(STATUS_META\[issue\.status\]\.labelKey\)/);
  assert.match(kanbanPage, /tone: granularLabel \? getIssueGranularStatusTone\(issue\) : issue\.status/);
  ...
  assert.doesNotMatch(kanbanPage, /function getIssueStageLabel/);
  ...
  assert.doesNotMatch(kanbanPage, /className="issue-card-context-line is-taxonomy"/);
  assert.doesNotMatch(kanbanPage, /ApartmentOutlined/);
  assert.match(kanbanPage, /<TagOutlined aria-hidden="true" \/>/);
  assert.match(kanbanPage, /className=\{`issue-card-status is-\$\{cardStatus\.tone\}`\}/);
  assert.doesNotMatch(kanbanPage, /issue-card-taxonomy-item/);
  assert.match(kanbanPage, /<section className="issue-card-section issue-card-title-block">/);
  assert.match(kanbanPage, /className="issue-card-title" title=\{issue\.title\}/);
  assert.doesNotMatch(kanbanPage, /<strong title=\{description \|\| issue\.title\}>/);
  assert.match(kanbanPage, /const descriptionPreview = getIssueDescriptionPreview\(issue\.description\)/);
  assert.match(kanbanPage, /className="issue-card-description" title=\{descriptionPreview \|\| undefined\}/);
  ...
  assert.match(kanbanPage, /className=\{`issue-card-signal issue-card-footer-signal is-\$\{cardSignal\.tone\}`\}/);
  ...
  assert.match(kanbanPage, /issue-card-footer-signal/);
  assert.match(kanbanPage, /const duePresentation = getIssueCardDuePresentation\(issue, locale, now, t\)/);
  assert.match(kanbanPage, /const overdue = issue\.status !== "completed" && issue\.runState !== "completed" && issue\.dueAt < now\.getTime\(\)/);
  assert.match(kanbanPage, /className=\{`issue-card-due \$\{duePresentation\.overdue \? "is-overdue" : ""\}`\}/);
  assert.match(kanbanPage, /duePresentation \? \([\s\S]{0,500}: display\.priority \? \(/);
  assert.match(kanbanPage, /className="issue-card-footer-start"/);
  assert.match(kanbanPage, /className="issue-card-footer-badges"/);
  assert.match(kanbanPage, /className=\{`issue-card-footer-end has-\$\{actionCount\}-actions`\}/);
  ...
  assert.match(kanbanPage, /const shouldShowPeople = shouldShowAssignee \|\| Boolean\(peopleLine\.worker\)/);
  assert.doesNotMatch(kanbanPage, /display\.assignee/);
  ...
  assert.match(kanbanStyles, /\.issue-card-context\s*\{[\s\S]{0,180}min-height:\s*52px;/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-context-line\.is-taxonomy/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-taxonomy-item/);
  assert.match(kanbanStyles, /\.issue-card-title\s*\{[\s\S]{0,220}white-space:\s*nowrap;[\s\S]{0,180}font-weight:\s*500;/);
  assert.match(kanbanStyles, /\.issue-card-description\s*\{[\s\S]{0,260}-webkit-line-clamp:\s*2;[\s\S]{0,260}font-size:\s*11\.5px;/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-status-dot\s*\{/);
  assert.doesNotMatch(kanbanStyles, /\.issue-card-footer-signals\s*\{/);
  assert.match(kanbanStyles, /\.issue-card-footer-signal\s*\{[\s\S]{0,100}width:\s*100%;/);
  ...
  assert.match(zhCN, /"kanban\.card\.dueAt": "截止 \{time\}"/);
  ...
  assert.match(enUS, /"kanban\.card\.dueAt": "Due \{time\}"/);
  assert.match(enUS, /"kanban\.card\.overdueAt": "Overdue \{time\}"/);
...
test("Kanban toolbar merges issue count into the wider project filter and compacts cloud status", () => {
  const kanbanPage = readSourceFile("src", "renderer", "pages", "kanban", "KanbanPage.tsx");
  const kanbanStyles = readSourceFile("src", "renderer", "styles", "kanban.css");
